...
void setup() {
  ...
  for (int i = 0; i< imgsInfos[0].length; i++) {
    Particle p = new Particle(random(10, width-10), random(10, width-10), random(0, 1));//imgsInfos[0][i].pos.x, imgsInfos[0][i].pos.y, imgsInfos[0][i].size);
    p.setImgNo(-1);
    p.isExploded = true;
...
void draw() {
  ...
  if (mode == 0) {
    ...
    if (allExploded) {
      ...
      if (millis() - clickedMoment2 >= 1000*8) {
        allExploded = false;
        allNext = true;
        for (int i = 0; i < particles.length; i ++) {
          Particle p = particles[i];
