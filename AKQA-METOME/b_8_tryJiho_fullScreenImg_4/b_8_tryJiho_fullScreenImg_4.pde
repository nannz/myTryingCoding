...
void setup() {
  ...
  //logo.scale(0.2);
  File dir = new File(sketchPath(""), "../b_footage_Jiho");
  if (dir.isDirectory()) {
  ...
  for (int i = 0; i < imageCount; i ++) {
    ...
    imgTemp.loadPixels();
    float imgTempXoff = random(0, width - imgTemp.width);
    float imgTempYoff = random(0, height - imgTemp.height);
...
void draw() {
  ...
  if (mode == 0) {
    ...
    if (allExploded) {
      ...
      if (millis() - clickedMoment2 >= 1000*10) {
        allExploded = false;
        allNext = true;
        for (int i = 0; i < particles.length; i ++) {
          Particle p = particles[i];
    ...
    if (allNext) {
      for (int i = 0; i < particles.length; i ++) {
        ...
        if (p.isNext == true) {
          ...
          destDirection = destDirection.mult(distance.mag() * 0.01);
      ...
      if (millis() - clickedMoment >= 1000*7) {//explode!
        particleImgCount +=1;
