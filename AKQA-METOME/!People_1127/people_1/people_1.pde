/*tried rotate the particle system. Failed*/
...
float time4Flying = 2.5;
float time4FormingNext = 5.0;
void setup() {
  //size(1280, 720);
  size(1680, 1050);
  background(255);
    ...
    if (allNext) {
      ...
      if (millis() - clickedMoment >= 1000* time4FormingNext) {//explode!
        ...
        for (int i = 0; i < particles.length; i ++ ) {
          ...
          //print to test
          if (i == 0) {
            println("applied randomed force!");
          }
        }
      }
    }
